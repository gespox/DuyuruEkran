...
<div class="w3-main" style="margin-left:300px;margin-top:43px;">
    ...
    <header class="w3-container" style="padding-top:22px">
        <h5><b><i class="fa fa-dashboard"> </i> Resimsiz Duyuru </b></h5>
    </header>
    <!-- CONTENT CONTAINER -->
    <div class="w3-container w3-white w3-margin w3-padding">
        <div class="w3-container  w3-white w3-margin w3-padding ">
            ...
            if(isset($_POST["resimsizKayit"])) {
                ...
                $yazi = $_POST['yazi'];
                $bitis = $_POST['bitis'];
                $sqlresimsiz = "INSERT INTO o_resimsiz (kullanici_id,baslik,yazi,bitis) VALUES (?,?,?,?)";
                $resimsiz=$conn->prepare($sqlresimsiz);
                $checkResimsiz=$resimsiz->execute([$idKullanici,$baslik,$yazi,$bitis]);
                if($checkResimsiz) {
                    $resimsizEkran= $conn->lastInsertId();
                    $ekran_id=$_SESSION['ekran_id'];
                    $sql = "INSERT INTO ekran_resimsiz (ekran_id,resimsiz_id) VALUES (?,?)";
                    $kayitsql = $conn->prepare($sql);
                    $kayitsql->execute([$ekran_id,$resimsizEkran]);
                    echo "<div class='alert alert-success alert-dismissible fade show' role='alert'>
                         Resimsiz Duyuru Basariyla Eklendi!
        ...
        if(isset($_POST["sil"])){
            ...
            $sqlSil="DELETE FROM o_resimsiz WHERE id_resimsiz=?";
            $ResimsizSil=$conn->prepare($sqlSil);
            $checkSil= $ResimsizSil->execute([$resimsizid]);
            if($checkSil) {
                $sqlekrandansil="DELETE FROM ekran_resimsiz WHERE resimsiz_id=?";
        ...
        <div class="w3-container w3-margin w3-padding ">
            <table class="table table-hover">
                ...
                $sqlSelectResimsiz = "SELECT * FROM o_resimsiz WHERE kullanici_id=?";
                ...
                if ($preResimsiz->rowCount() > 0) {
                    while ($rowResimsiz = $preResimsiz->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        echo "<td>";?>
                        <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
                            <input type="hidden" name="resimsizID" value="<?php echo $rowResimsiz['id_resimsiz']?>">
